...
def _install_macos_stubs(
    ...
    """Stub the macOS surface the disconnect path drives, and return the file
    recording every `networksetup` invocation.
...
def _run(
    ...
    """Run the script against a stub CLI; return (proc, menu.log, set args, ns calls)."""
    ...
    bounces = ns_calls.read_text() if ns_calls.exists() else ""
    return proc, log, set_args, bounces
...
def test_healthy_set_passes_node_and_lan_flag(tmp_path: Path) -> None:
    proc, log, set_args, bounces = _run(tmp_path, "ca", ONLINE)
    ...
    assert "setairportpower" not in bounces, "connect path must never touch Wi-Fi"


def test_off_clears_exit_node_without_lan_flag(tmp_path: Path) -> None:
    proc, log, set_args, bounces = _run(tmp_path, "off", ONLINE)
    assert proc.returncode == 0, proc.stderr
    assert "off" in log
    assert set_args == ["--exit-node="]
    assert "setairportpower" not in bounces, "a route that never drops needs no bounce"
...
def test_route_present_at_t0_then_gone_is_not_treated_as_clean(tmp_path: Path) -> None:
    ...
    """
    proc, log, set_args, bounces = _run(
        tmp_path, "off", ONLINE, routers=["192.168.1.1", "-"]
    )
    assert set_args == ["--exit-node="], "the disconnect itself must still happen"
    assert "setairportpower en0 off" in bounces, "must bounce the captured interface"
...
def test_route_returning_on_its_own_avoids_the_wifi_bounce(tmp_path: Path) -> None:
    ...
    assert "setairportpower" not in bounces
...
def test_bounce_recovers_the_route(tmp_path: Path) -> None:
    ...
    assert proc.returncode == 0, proc.stderr
    assert set_args == ["--exit-node="]
    assert "setairportpower en0 off" in bounces and "setairportpower en0 on" in bounces
