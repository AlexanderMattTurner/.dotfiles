...
- **Writing `[[ "$(uname)" == Darwin ]]` / `$IS_MAC` opts that branch out of CI.** A single-OS test job skips the whole branch, so the suite stays green having never run a line of it. Decide coverage at the moment you add the gate, not later: either stub `uname` and the platform-only commands onto `PATH` so the branch executes on the CI OS (see the `writing-tests` skill), or add the second OS to the job matrix.
